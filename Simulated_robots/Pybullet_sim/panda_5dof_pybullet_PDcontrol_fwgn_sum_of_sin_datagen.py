"""
Pybullet simulation of the Franka Emika Panda robot restricted to 5DOF,
...
parser = argparse.ArgumentParser('Franka Emika Panda (5DOF) PD vel controller')
...
parser.add_argument('-robot_name',
                    type=str,
                    default='FE_panda5dof_pybul',
                    help='name of the simulated robot')

parser.add_argument('-robot_urdf',
                    type=str,
                    default='./FE_panda/models/panda_arm_5dof.urdf',
...
ll = [-2.9, -1.8, -2.9, -3.0, -2.9] #[rad] -- last value is for joint 8 which is fixed
ul = [2.9, 1.8, 2.9, 0.0, 2.9] #[rad] -- last value is for joint 8 which is fixed
...
for run in ['train', 'test']:
    ...
    if run == 'train':
        ...
        std_scalings = 1.4 * np.diag(np.array([1, 1, 1, 1, 1.4]))
