...
ll = [-2.9, -1.8, -2.9] #[rad] -- last value is for joint 8 which is fixed
ul = [2.9, 1.8, 2.9] #[rad] -- last value is for joint 8 which is fixed
...
for run in ['train', 'test']:
    ...
    std_scalings = 2.8*np.diag(np.array([1, 1, 1]))
    std_vec = np.dot(std_scalings, np.array([rang for rang in ranges]))
    #std_vec = 3*np.array([rang for rang in ranges[:-1]])
    q_ref, dq_ref = get_filtered_wgn_trj(num_steps, dt, num_joints, rng, mean_ranges, std_vec, fc)
