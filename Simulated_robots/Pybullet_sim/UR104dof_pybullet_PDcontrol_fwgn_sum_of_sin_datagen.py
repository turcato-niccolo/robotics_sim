...
ll = [-(3 / 2) * math.pi] * num_joints  # [rad] -- last value is for joint 8 which is fixed
ul = [(3 / 2) * math.pi] * num_joints  # [rad] -- last value is for joint 8 which is fixed
ranges = [ul[i] - ll[i] for i in range(num_joints)]
mean_ranges = np.array([(ul[i] + ll[i]) / 2 for i in range(num_joints)])
max_dq = [(2 / 3) * math.pi] * 2 + [math.pi] * 2
max_ddq = [10] * num_joints  # Try less acceleration

# Set the robot rest pose
rest_pose = np.zeros((num_joints, 1))

# Set the joint max torque
max_torques = [330] * 2 + [150]  + [56] #* 3  # N
