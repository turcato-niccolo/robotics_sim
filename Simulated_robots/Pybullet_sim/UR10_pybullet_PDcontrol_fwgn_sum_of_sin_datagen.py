...
# Spawn the robot
ur10Id = pybullet.loadURDF(robot_urdf, [0, 0, 0], flags=pybullet.URDF_USE_SELF_COLLISION , useFixedBase=True)

# Print useful debug info
print('Plane Body ID = ', planeId)
print('Robot Body ID = ', ur10Id)

# reset the position of the robot base
pybullet.resetBasePositionAndOrientation(ur10Id, [0, 0, 0], [0, 0, 0, 1])

# Store the number of joints
num_joints = pybullet.getNumJoints(ur10Id) #-2

# Set the dynamical properties for each link (friction and damping coeffs)
for i in range(num_joints):
    pybullet.changeDynamics(bodyUniqueId=ur10Id,
                            linkIndex=i,
                            lateralFriction=0.0,
...
for i in range(num_joints):
    pybullet.setJointMotorControl2(ur10Id, i, pybullet.POSITION_CONTROL, targetPosition=0, force=0)

# Define joint operational ranges
ll = [-(3/2)*math.pi]*3 + [-math.pi]*3  # [rad] -- last value is for joint 8 which is fixed
ul = [ (3/2)*math.pi]*3 + [ math.pi]*3 # [rad] -- last value is for joint 8 which is fixed
ranges = [ul[i] - ll[i] for i in range(num_joints)]
mean_ranges = np.array([(ul[i] + ll[i]) / 2 for i in range(num_joints)])
max_dq = [(2/3)*math.pi]*3 + [math.pi]*3
max_ddq = [20] * num_joints # Try less acceleration

# Set the robot rest pose
rest_pose = np.zeros((num_joints, 1))

# Set the joint max torque
max_torques = [330.0] * 2 + [150.0] + [0.5] * 3  # N
...
for run in ['train', 'test']:
    ...
    if run == 'train':
        ...
        std_scalings = np.diag(np.array([1.4]*3 + [1]*3))
    ...
    for i in range(num_joints):
        pybullet.resetJointState(ur10Id, i, q_ref[0, i])
    ...
    for k in range(num_steps):
        ...
        js = pybullet.getJointStates(ur10Id, list(range(num_joints)))
        ...
        g_k = pybullet.calculateInverseDynamics(ur10Id, q_k, [0] * (num_joints), [0] * (num_joints))

        c_k = pybullet.calculateInverseDynamics(ur10Id, q_k, dq_k, [0] * (num_joints))
        ...
        M_k = pybullet.calculateMassMatrix(ur10Id, q_k)
        M_k_prev = M_k
        M.append(M_k)
        g.append(g_k)
        c.append(c_k)
        ...
        pybullet.setJointMotorControlArray(ur10Id,
