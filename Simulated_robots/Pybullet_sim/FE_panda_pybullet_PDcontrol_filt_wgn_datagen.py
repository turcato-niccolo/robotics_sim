...
for run in ['train', 'test']:
    ...
    fc = 0.7  # [Hz] cutting frequency of the filter
    std_scalings = 2.8*np.diag(np.array([1, 1, 1, 1, 1.5, 1.5, 1.5]))
    std_vec = np.dot(std_scalings, np.array([rang for rang in ranges[:-1]]))
    #std_vec = 3*np.array([rang for rang in ranges[:-1]])
    q_ref, dq_ref = get_filtered_wgn_trj(num_steps, dt, num_joints-1, rng, mean_ranges[:-1], std_vec, fc)
    ...
    if run == 'train':
        ...
        tau_tr_std = [np.std(tau_tr[:, i]) for i in range(num_joints-1)]
        tr_noise_std = np.array(tau_tr_std) * 5e-2

        tau_tr_noised = noising_signals(tau_tr, tr_noise_std, [0]*(num_joints-1))
...
if flg_save:
    ...
    tau_noised_names = ['tau_noised_'+str(k+1) for k in range(0,num_joints-1)]
    data_names = q_ref_names + dq_ref_names + q_names + dq_names + ddq_names + tau_names + m_names + c_names + g_names

    # Create the dataframes
    data_frame_tr = pd.DataFrame(data=np.concatenate([q_ref_tr, dq_ref_tr, q_tr, dq_tr, ddq_tr, tau_tr, m_tr, c_tr, g_tr, tau_tr_noised], axis=1), columns=data_names+tau_noised_names)
    data_frame_test = pd.DataFrame(data=np.concatenate([q_ref_test, dq_ref_test, q_test, dq_test, ddq_test, tau_test, m_test, c_test, g_test],axis=1), columns=data_names)

    # Store the dataframe as a pkl file 
    pkl.dump(data_frame_tr, open(saving_path+robot_name+'_fwgn_tr.pkl', 'wb'))
    pkl.dump(data_frame_test, open(saving_path+robot_name+'_fwgn_test.pkl', 'wb'))

    # Store the Inertial matrices
    pkl.dump(M_tr, open(saving_path+robot_name+'_M_tr.pkl', 'wb'))
    pkl.dump(M_test, open(saving_path+robot_name+'_M_test.pkl', 'wb'))
