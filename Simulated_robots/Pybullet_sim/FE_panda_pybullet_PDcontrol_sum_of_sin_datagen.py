""" 
Pybullet simulation of the Franka Emika Panda robot, 
...
from datagen import *
#%%
#Custom funct definition

#%% Set command line args

parser = argparse.ArgumentParser('Franka Emika Panda PD vel controller')
...
parser.add_argument('-robot_urdf',
                    type=str,
                    default='./FE_panda/models/panda_arm.urdf',
...
num_joints = pybullet.getNumJoints(pandaId)  
...
ll = [-2.9, -1.8, -2.9, -3.0, -2.9, 0.0, -2.9, 0.0] #[rad] -- last value is for joint 8 which is fixed
ul = [2.9, 1.8, 2.9, 0.0, 2.9, 3.8, 2.9, 0.0] #[rad] -- last value is for joint 8 which is fixed
...
max_torques = [87]*4 + [12]*3 # N

#%% 

# Get the reference trajectory -- a circle performed by link 7 (the end effector)
#ee_link_idx = 7 

# y_ref = 0.2*np.sin(2*np.pi*(1/num_steps/dt)*t)
# x_ref = 0.5*np.ones((num_steps,))
# z_ref = 0.5 + 0.2*np.sin(2*np.pi*(1/num_steps/dt)*t)
# p_ref = np.array([x_ref, y_ref, z_ref]).T 
# q_ref = []
# for i in range(num_steps):
#     q_ref.append(pybullet.calculateInverseKinematics(pandaId, 
#                                                      ee_link_idx, 
#                                                      list(p_ref[i]),
#                                                      lowerLimits=ll,
#                                                      upperLimits=ul,
#                                                      jointRanges=ranges,
#                                                      restPoses=rest_pose))
# # Transform q_ref in a numpy array
# q_ref = np.array([np.array(xi) for xi in q_ref])

# # Compute dq_ref
# dq_ref = np.zeros([num_steps, num_joints-1])
# for k in range(1, num_steps):
#     dq_ref[k,:] = (q_ref[k,:] - q_ref[k-1,:])/dt
# dq_ref[0,:] = dq_ref[1,:]



#%% Set the controller gains
# positionGains = [1000, 1000, 1000, 1000, 1000, 1000, 1000]
# velocityGains = [ 100,  100,  100,  100,  100,  100,  100]
# positionGains = [10, 10, 10, 10, 10, 10, 10]
# velocityGains = [ 1,  1,  1,  1,  1,  1,  1]
scaling = 0.1
positionGains = [1*scaling]*(num_joints-1)
velocityGains = [10*scaling]*(num_joints-1)
...
for run in ['train', 'test']:
    ...
    # Get the reference trajectories: sum of sinusoids
    max_freq = 0.5
    num_freq = 10
    max_q = np.abs(np.array(ranges)/2)
    q_ref, dq_ref, ddq_ref = get_sum_of_sinusoids_trj(num_steps, dt, num_joints-1, rng,max_freq, num_freq, max_q, mean_ranges[:-1], max_dq, max_ddq)
    ...
    for k in range(num_steps):
        # Initialize list to store the current pos vel and torque
        q_k = []
        dq_k = []
        tau_k = []
        M_k = []
        # Get current states from pybullet
        js = pybullet.getJointStates(pandaId, list(range(num_joints - 1)))
        q_k = [js[i][0] for i in range(num_joints - 1)]
        dq_k = [js[i][1] for i in range(num_joints - 1)]
        tau_k = [js[i][-1] for i in range(num_joints - 1)]
        q.append(q_k)
        dq.append(dq_k)
        tau.append(tau_k)

        g_k = pybullet.calculateInverseDynamics(pandaId, q_k, [0]*(num_joints-1), [0]*(num_joints-1))

        c_k = pybullet.calculateInverseDynamics(pandaId, q_k, dq_k, [0] * (num_joints - 1))
        c_k = [c_k[i]-g_k[i] for i in range(num_joints - 1)]
    ...
    # Convert q and dq into numpy arrays
    if run == 'train':
        ...
        M_tr = np.zeros([num_steps, num_joints - 1, num_joints - 1])
        for i in range(num_steps):
            M_tr[i, :, :] = np.array(M[i])
        ...
        # q_ref_tr = q_ref
        # dq_ref_tr = dq_ref
        # q_tr = np.array([np.array(q_k) for q_k in q])
        # dq_tr = np.array([np.array(dq_k) for dq_k in dq])
        # ddq_tr = np.zeros(dq_tr.shape)
        # ddq_tr[0,:] = (dq_tr[1,:] - dq_tr[0,:])/dt
        # ddq_tr[1:-1,:] = (dq_tr[2:,:] - dq_tr[:-2])/(2*dt) 
        # ddq_tr[-1,:] = (dq_tr[-1,:] - dq_tr[-2,:])/dt
        # tau_tr = np.array([np.array(tau_k) for tau_k in tau])
        ...
        M_test = np.zeros([num_steps, num_joints - 1, num_joints - 1])
        ...
        # q_ref_test = q_ref
        # dq_ref_test = dq_ref
        # q_test = np.array([np.array(q_k) for q_k in q])
        # dq_test = np.array([np.array(dq_k) for dq_k in dq])
        # ddq_test = np.zeros(dq_test.shape)
        # ddq_test[0,:] = (dq_test[1,:] - dq_test[0,:])/dt
        # ddq_test[1:-1,:] = (dq_test[2:,:] - dq_test[:-2])/(2*dt) 
        # ddq_test[-1,:] = (dq_test[-1,:] - dq_test[-2,:])/dt
        # tau_test = np.array([np.array(tau_k) for tau_k in tau])
...
if flg_save:
    ...
    q_ref_names = ['q_ref_'+str(k+1) for k in range(0,num_joints-1)]
    dq_ref_names = ['dq_ref_'+str(k+1) for k in range(0,num_joints-1)]
    q_names = ['q_'+str(k+1) for k in range(0,num_joints-1)]
    dq_names = ['dq_'+str(k+1) for k in range(0,num_joints-1)]
    ddq_names = ['ddq_'+str(k+1) for k in range(0,num_joints-1)]
    tau_names = ['tau_'+str(k+1) for k in range(0,num_joints-1)]
    m_names = ['m_' + str(k + 1) for k in range(0, num_joints - 1)]
    c_names = ['c_' + str(k + 1) for k in range(0, num_joints - 1)]
    g_names = ['g_' + str(k + 1) for k in range(0, num_joints - 1)]
    ...
    data_frame_tr = pd.DataFrame(
        data=np.concatenate([q_ref_tr, dq_ref_tr, q_tr, dq_tr, ddq_tr, tau_tr, m_tr, c_tr, g_tr], axis=1),
        columns=data_names)
    data_frame_test = pd.DataFrame(
        data=np.concatenate([q_ref_test, dq_ref_test, q_test, dq_test, ddq_test, tau_test, m_test, c_test, g_test], axis=1),
        columns=data_names)

    # Store the dataframe as a pkl file
    pkl.dump(data_frame_tr, open(saving_path+robot_name+'_sum_of_sin_tr.pkl', 'wb'))
    pkl.dump(data_frame_test, open(saving_path+robot_name+'_sum_of_sin_test.pkl', 'wb'))

    # Store the Inertial matrices
    pkl.dump(M_tr, open(saving_path+robot_name+'_M_sum_of_sin_tr.pkl', 'wb'))
    pkl.dump(M_test, open(saving_path+robot_name+'_M_sum_of_sin_test.pkl', 'wb'))
#%% 
