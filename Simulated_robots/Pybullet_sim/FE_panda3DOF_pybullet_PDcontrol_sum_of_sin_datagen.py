...
parser.add_argument('-robot_name',
                    type=str,
                    default='FE_panda3DOF_pybul',
                    help='name of the simulated robot')

parser.add_argument('-robot_urdf',
                    type=str,
                    default='./FE_panda/models/panda_arm_3dof_amp.urdf',
...
# # Compute dq_ref
# dq_ref = np.zeros([num_steps, num_joints])
...
for run in ['train', 'test']:
    ...
    q_ref, dq_ref, ddq_ref = get_sum_of_sinusoids_trj(num_steps, dt, num_joints, rng,max_freq, num_freq, max_q, mean_ranges, max_dq, max_ddq)
...
plt.figure()
plt.plot(t, tau_test[:, 1], label=r'$tau\_{test}$')
plt.plot(t, g_test[:, 1], label=r'$g\_{test}$')
plt.plot(t, m_test[:, 1], label=r'$m\_{test}$')
plt.plot(t, c_test[:, 1], label=r'$c\_{test}$')
plt.plot(t, tau_test_reconstr[:, 1], label=r'$taurec\_{test}$')
